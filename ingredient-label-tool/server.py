...
def normalize_payload(payload: object) -> dict[str, object]:
    ...
    for row in rows_value:
        ...
        rows.append({"name": name, "amount": amount, "dailyValue": daily_value or "**"})
    if not rows:
        raise ValueError("请至少填写一行成分信息。")
...
def generate_pdf(payload: object, output_path: Path) -> dict[str, object]:
    ...
    outer_bottom_offset = BASE_OUTER_BOTTOM_OFFSET + row_delta
    ...
    canvas.drawString(
        ...
        f"Serving Per Container {data['servingsPerContainer']}",
    )
