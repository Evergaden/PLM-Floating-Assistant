...
// @version      1.4.0
// @description  可选的噜噜乐园皮肤资源：趋势卡片趴趴轮换彩蛋和加载贴纸。
// @author       Violet
// @match        https://plm.westmonth.com/*
// @match        https://auth.westmonth.com/*
// @downloadURL  https://plm-cloud-backup.wt196731.workers.dev/assets/v5/plm-lulu-theme.user.js
// @updateURL    https://plm-cloud-backup.wt196731.workers.dev/assets/v5/plm-lulu-theme.user.js
...
(function () {
  ...
  const LULU_SKIN_CSS = `
    ...
    #plm-floating-helper[data-pfh-theme="lulu"] .pfh-home-welcome > span {
      color: var(--pfh-theme-primary-hover) !important;
      background: var(--pfh-theme-primary-soft) !important;
    ...
    #plm-floating-helper[data-pfh-theme="lulu"] .pfh-home-analytics {
      overflow: visible !important;
    }
    #plm-floating-helper[data-pfh-theme="lulu"] .pfh-lulu-home-hover,
    #plm-floating-helper[data-pfh-theme="lulu"] .pfh-lulu-hover {
      display: none !important;
    }
  ...
  function nextPerchImageUrl() {
    const url = LULU_PERCH_IMAGE_URLS[perchImageIndex % LULU_PERCH_IMAGE_URLS.length];
    perchImageIndex = (perchImageIndex + 1) % LULU_PERCH_IMAGE_URLS.length;
    return url;
  }

  function stopPerchRotation() {
    if (!perchRotateTimer) return;
    window.clearInterval(perchRotateTimer);
    perchRotateTimer = 0;
  }

  function ensurePerchRotation() {
    if (perchRotateTimer) return;
    perchRotateTimer = window.setInterval(() => {
      const panel = document.getElementById(PANEL_ID);
      const image = panel && panel.dataset.pfhTheme === 'lulu'
        ? panel.querySelector('.pfh-lulu-card-perch img')
        : null;
      if (!image) {
        if (!panel || panel.dataset.pfhTheme !== 'lulu') stopPerchRotation();
        return;
      }
      image.src = nextPerchImageUrl();
    }, 15000);
  }
  ...
  function createMascot(className) {
    ...
    image.src = className === 'pfh-lulu-card-perch' ? nextPerchImageUrl() : LULU_IMAGE_DATA_URL;
    image.alt = '';
  ...
  function syncMascots() {
    ...
    const legacyMascots = panel.querySelectorAll('.pfh-lulu-home-hover, .pfh-lulu-hover');
    legacyMascots.forEach((node) => node.remove());
    const perchMascots = panel.querySelectorAll('.pfh-lulu-card-perch');
    if (!enabled) {
      perchMascots.forEach((node) => node.remove());
      stopPerchRotation();
      return;
    }
    ...
    if (chartCard) ensurePerchRotation();
  }
