...
async function handleLoadingTipsBulkSave(request, env) {
  ...
  return adminRedirect('/admin?saved=tips#tips');
}
...
function renderAdminDashboardPage(users, dashboard, campaigns, holidays, saved) {
  ...
  return '<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>PLM 管理后台</title><style>' +
    ...
    (saved ? '<div class="notice">' + (saved === 'holidays' ? '节假日设置已保存' : '轮播小提示与推送条件已保存') + '</div>' : '') +
    ...
    '<section class="card" id="saved-tips"><div class="cardhead"><h2>已保存的小提示（' + campaigns.length + '）</h2><div class="sub">展开任意一条进行单独维护，最后统一保存</div></div><form class="form" method="post" action="/admin/tips/manage-save"><input type="hidden" name="tipCount" value="' + campaigns.length + '"><div class="tiplist">' + tipEditorRows + '</div><label><span>新增提示（每行一条）</span><textarea name="newTexts" placeholder="在这里追加新的提示，不会覆盖上面的内容"></textarea></label><div class="actions"><button type="submit">保存全部修改</button></div></form></section>' +
    '<section class="card" id="holidays"><div class="cardhead"><h2>法定节假日</h2><div class="sub">每行格式：节日名称|放假开始日期，提醒日期自动取前一天</div></div><form class="form" method="post" action="/admin/holidays/bulk-save"><textarea name="holidays" placeholder="国庆节|2026-10-01">' + htmlEscape(holidayTexts) + '</textarea><div class="actions"><button type="submit">保存节假日</button></div></form></section></div></main><script>document.querySelectorAll("form").forEach(function(form){form.addEventListener("submit",function(){var button=form.querySelector("button[type=submit],button:not([type])");if(button){button.disabled=true;button.textContent="保存中..."}})});</script></body></html>';
}
